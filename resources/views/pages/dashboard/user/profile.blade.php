...
@section('style')
    <style>
        ...
        .profile-container:hover .profile_edit_icon {
            display: flex;
			color: #555;
			cursor: pointer;
        }
...
@endsection
@section('page')
<div class="profile-container" style="background: url({{'/front/img/'.auth()->user()->bg_img??'../../theme/media/image/image1.jpg'}})">
	<div class="d-flex align-items-center">
		<figure class="avatar avatar-lg mr-3">
			<img id="profile_avatar" src="{{auth()->user()->image()}}"
    ...
    <form id="profileUploadForm">
        <label for="image_profile" class="profile_edit_icon"><i class="fa fa-pencil fa-lg"></i></label>
		<input type="file" id="image_profile" name="image_profile" accept="image/*" class="input-file">
		<button type="submit" class="input-file" id="submit"></button>
    </form>
</div>
		...
		<div class="card">
			<div class="card-body">
				...
				<ul class="list-group list-group-flush">
					@foreach ($followList as $like)
					<li class="list-group-item d-flex align-items-center px-0">
						<figure class="avatar avatar-state-success mr-3">
							<img src="{{$like->user->image()}}" class="rounded-circle" alt="image">
						</figure>
						<div>
							<h6 class="mb-0">{{$like->user->name}}</h6>
							<small class="text-muted">{{$like->user->userType()}}</small>
						</div>
					</li>
					@endforeach
				</ul>
			...
			<div class="card-body">
				...
				<div class="d-flex align-items-center">
					<div class="avatar-group mr-2">
						@foreach ($friendList as $key => $user)
							@if ($key < 5)
							<a href="#" class="avatar" data-toggle="tooltip" title="{{$user->friend->name}}">
								<img src="{{$user->friend->image()}}" class="rounded-circle" alt="image">
							</a>
							@endif
						@endforeach
					</div>
    ...
    var readURL = function(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                $('.profile-container').css('background', 'url(' + imageUrl + ')');
            }
            reader.readAsDataURL(input.files[0]);
        }
    }

	$(document).on('submit', '#profileUploadForm', function(e) {
		e.preventDefault();

		$.ajax({
			url: '/dashboard/profile/updateProfileImage',
			method: 'POST',
